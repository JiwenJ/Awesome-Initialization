...
The following direct HPT papers remain in their original collection and are not duplicated in the 27-paper count above.
...
The public-source audit covers records available through **2026-09-24**. It searched exact hyperparameter-transfer terminology, learning-rate and batch-size scaling, model/data/token/sparsity laws, parameterization methods, normalized and hypersphere training, citations in recent papers, and official code releases. The latest pass adds a small-model Step Law recalibration, optimizer-memory horizon evidence, configuration-aware surrogate selection, and historical batch/EMA/compute-dependent rules. Indexed search cannot prove absolute completeness, and several 2026 items remain preprints.

See the [2026-09-24 search audit](search-audit-2026-09-24.md) for this update's evidence and decisions, and the [2026-09-19 search audit](hyperparameter-transfer-search-audit-2026-09-19.md) for the preceding queries, false positives, code checks, and μP/Hyperball review. Follow [CONTRIBUTING.md](../CONTRIBUTING.md) when adding records.
